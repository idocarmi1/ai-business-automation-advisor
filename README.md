...
## הגנת דמו על עמוד ניהול פניות

עמוד `ניהול פניות` מוגן בפרויקט הדמו לפי כתובת אימייל של מנהל:

`autobiz.advisor.ai@gmail.com`

רק משתמש שמחובר עם כתובת זו יראה את פריט הניווט `ניהול פניות` ויוכל לפתוח את העמוד דרך:

`https://ai-business-automation-advisor.vercel.app/#admin-leads`

אם משתמש רגיל או מבקר לא מחובר פותח את הקישור, האתר לא יציג את עמוד ניהול הפניות ויציג הודעת הרשאה בעברית. זוהי הגנת דמו בלבד. בגרסת Production יש להשתמש באימות משתמשים אמיתי בצד שרת, הרשאות תפקידים, Session מאובטח ו-Role-Based Access Control.
